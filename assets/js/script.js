...
function AudioPlayer() {
    ...
    this.audio.id = 'mainAudioPlayer';
    this.audio.style.display = 'none';
    document.body.appendChild(this.audio);
    // Set audio element attributes for proper playback
    this.audio.preload = 'metadata';
    // Removed crossOrigin = 'anonymous' to prevent CORS issues with external sources

    this.setTrack = function(track) {
        this.currentlyPlaying = track;
        // Support both legacy local files (path) and Jamendo API tracks (audio)
        // Jamendo tracks have an 'audio' property (usually a URL)
        // Legacy local tracks have a 'path' property
        if (track.audio) {
            // Jamendo API track - use the audio URL
            console.log('Setting Jamendo track:', track.audio);
            this.audio.src = track.audio;
        } else if (track.path) {
            // Legacy local file path
            console.log('Setting local track:', track.path);
            this.audio.src = track.path;
        } else {
            console.error("Track object missing both 'audio' and 'path' properties", track);
        }
        this.audio.load();
    }
    ...
    this.play = function() {
        ...
        this.audio.play().catch(error => {
            ...
            // alert('Playback blocked or failed: ' + error.message);
        });
    }
    ...
    this.audio.addEventListener('error', function(e) {
        console.error('Audio loading error:', e, 'Current src:', this.src);
    });
